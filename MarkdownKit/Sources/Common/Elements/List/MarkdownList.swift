...
final public class GRMarkdownList: MarkdownList {
    public var indicatorLevel2: String = "∙"
    public var indicatorLevel3: String = "∙"

    override public func formatText(_ attributedString: NSMutableAttributedString, range: NSRange, level: Int) {
        let levelIndicatorList = [
            1: "\(indicator)  ",
            2: "\(indicator)  ",
            3: "\(indicatorLevel2)  ",
            4: "\(indicatorLevel2)  ",
        ...
        guard
            let indicatorIcon = levelIndicatorList[level],
            let offset = levelIndicatorOffsetList[level]
        else { return }
        let indicator = "\(offset)\(indicatorIcon)"
        attributedString.replaceCharacters(in: range, with: indicator)
        attributedString.addAttributes([.paragraphStyle : defaultParagraphStyle()], range: range)
